...
def convert_to_coco_in_list(det_result,):
    id_ = 0
    annotations = []
    for k,item in enumerate(det_result):
        #convert image id to integer by defaults
        ...
        if 'categories' in item:
            ...
            categoreis = []
        for class_id, score,box in zip(categories, scores, boxes,):
            area = (box[3] - box[1]) * (box[2] - box[0])
            box [2] = box[2] - box[0]
            box[3] = box[3] - box[1]
            annot = {"category_id":class_id, "bbox":box, "image_id":image_id, "iscrowd":False, "area": area, "id":id_, "score":score,}
            id_ += 1
            annotations.append(annot)
    return annotations
def main():
